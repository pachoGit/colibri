...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    {
	echo "<script>alert('ERROR: Agregue al menos un curso');window.location.href = '".base_url()."/index.php/alumnoPorCurso/registrar';</script>";
    }
	...
	curl_setopt_array($curl, array(
            CURLOPT_URL => base_url()."/index.php/alumnoPorCurso/create",
	...
	if ($data["Estado"] != 200)
	{
	    ...
	    echo "<script>alert('".$mensaje."');window.location.href = '".base_url()."/index.php/alumnoPorCurso/registrar';</script>";
	}
    }
    $mensaje = $data["Detalles"];
...
<script type="text/javascript">
 ...
 function agregarCurso()
 {
     ...
     idcurso = document.forms[0].id_curso.value;
     traerCurso(idcurso, function(respuesta) {
	 var curso = JSON.parse(respuesta);

	 var existeCurso = document.getElementById(curso.idCurso);
	 if (existeCurso != null)
	 {
	     alert("Este curso ya esta en la tabla");
	     return;
	 }
		...
		<div class="widget-box">
			...
			<form  method="post" class="needs-validation" novalidate>
			    ...
			    <button  type="submit" class="btn btn-primary">Registrar</button>
                            <a href="<?= base_url().'/index.php/alumnoPorCurso/listar'; ?>" class="btn btn-danger"> Cancelar </a>
			</form>
